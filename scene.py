...
def combat_desbyte(player):
    ...
    print(f"{player.name}: I have to solve their question to get him back.")
    time.sleep(2)
...
def retrieve_the_codes(player):
    ...
    while user_check != "y" or user_check != "n":
...
def Final_mission(player):
    ...
    if player.call == True:
        ...
        print(f"{palyer.name}: I'm so glad I was able to save you")
        time.sleep(1)
